...
@override
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('English word frequency ranker'),
      ),
  ...
  String content = await rootBundle.loadString("assets/20k.txt");
    setState(() {
                  _data = [];
                });

  if (words.isNotEmpty || content.split("\n").length != 0) 
    {
...
List<DataRow> _buildRows() {
  return _data.skip(1).map((row) {
    ...
    Color rowColor = rankValue < 3500 ? Colors.blue: Colors.transparent;
